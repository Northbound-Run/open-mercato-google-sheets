// VENDORED from @open-mercato/core communication_channels/lib/oauth-token.
//
// This package must run in downstream apps where `@open-mercato/channel-gmail` and the
// `communication_channels` internal path may be absent, so we replicate the ~30-line
// token helper here rather than importing a core-internal module. The Google OAuth2
// token response shape, the form-urlencoded token POST, and the expiry computation are
...
/**
 * POST a form-urlencoded body to an OAuth token endpoint and return the parsed token
 * response. Throws `${errorLabel}: <reason>` when the endpoint returns a non-2xx status,
 * an `error` field, a non-JSON body, or does not respond within the timeout. Bounding
 * the request matters because token refresh sits on the critical path of every sync run —
 * a hung token endpoint must fail fast, and a proxy HTML error page must surface the real
 * status rather than a confusing JSON SyntaxError.
 */
