// Ambient shims for framework runtime dependencies that the consuming Open Mercato app
// provides but which are NOT installed in this standalone package. They exist only so `tsc`
// can resolve the framework's own source (the @open-mercato/* packages ship their types as
// .ts, so a deep import pulls in their internal imports too) without this package having to
// replicate the app's entire dependency graph. This package's own code never imports any of
// these directly — `awilix` (used by di.ts) is a real devDependency, not shimmed.
...
// Modules whose types the framework uses structurally (as generic types), so a bare `any`
// module isn't enough — provide the minimal shapes the framework's source references.
